...
export const verifyEmail = asyncHandler(async (req, res) => {
  ...
  if (user.isVerified) {
    return res.status(400).json({ message: "Пользователь уже подтверждён" });
  }
...
export const forgotPassword = asyncHandler(async (req, res) => {
  ...
  //create reser token using user id ---> expires in 1 hour
  ...
  //send email to user
  const subject = "Password Reset - TaskTracker";
  const send_to = user.email;
  const send_from = process.env.USER_EMAIL;
  ...
  if (!user.email) {
    console.error("User email is undefined or empty!");
  }

  try {
    await sendEmail(send_to, send_from, name, subject, template, reply_to, url);
